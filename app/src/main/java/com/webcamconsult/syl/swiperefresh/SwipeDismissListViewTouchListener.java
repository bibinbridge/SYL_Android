...
package com.webcamconsult.syl.swiperefresh;
...
public class SwipeDismissListViewTouchListener implements View.OnTouchListener {
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_UP: {
			...
			Log.e("viewwidth getting","-->"+( mViewWidth ));
			if(deltaX>0)
				dismiss=false;

			Log.e("viewwidth getting","-->"+( mViewWidth / 2));
			...
			if (dismiss && mDownPosition != ListView.INVALID_POSITION) {
				...
				.setListener(new AnimatorListenerAdapter() {
					...
					public void onAnimationEnd(Animator animation) {
						performDismiss(downView, downPosition,right);
					}
				});
			} else {
				// cancel
		...
		case MotionEvent.ACTION_MOVE: {
			...
			Log.e("slop,negative slop","-->"+mSlop+",--->"+(-mSlop));
			if (Math.abs(deltaX) > mSlop && Math.abs(deltaY) < (Math.abs(deltaX) / 2)   && deltaX<0       ) {
				// if (deltaX > (-mSlop) && deltaY < (-deltaX / 2)) {
			...
			if (mSwiping) {
				...
				Log.e("Translation",""+(deltaX - mSwipingSlop));
	...
	private void performDismiss(final View dismissView, final int dismissPosition,boolean right) 
