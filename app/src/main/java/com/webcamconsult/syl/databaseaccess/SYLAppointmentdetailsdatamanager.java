...
public class SYLAppointmentdetailsdatamanager {
	SYLDatabase syldatabase;

	/** The context. */
	Context context;

	public SYLAppointmentdetailsdatamanager() {

	}

	public SYLAppointmentdetailsdatamanager(Context context) {
	...
	public void insertappointmentsetails(
		...
		if (appointmentdetails != null && appointmentdetails.size() > 0) {
			...
			for (SYLFetchAppointmentsDetails syl_appointmentdetails : appointmentdetails) {
				...
				this.insertAppointmentdata(syl_appointmentdetails,
	...
	private void insertAppointmentdata(SYLFetchAppointmentsDetails sylappdetails,
		...
		try {
			...
			db.insert("appointment_details", null, values);
			db.close();
		} catch (Exception e) {
	...
	public ArrayList<SYLFetchAppointmentsDetails> getAllAppointmentDetails(
			String app_type) {
		String query;
...
String		query = "delete from appointment_details where appointment_type='"
				+ appointmenttype + "'                ";
		if (syldatabase != null) {
			try {
				SQLiteDatabase db = syldatabase.getWritableDatabase();
	...
	public  SYLFetchAppointmentsDetails getAllAppointmentFetchDetails(
...
Log.e("Error Message",e.getMessage());
	...
	public void insertcalendarEvevtIDs(String appointmentid,String eventid,
			Context c) {
		SQLiteDatabase db=null;
		try {
			if (syldatabase == null)
 ...
 db = syldatabase.getWritableDatabase();
			...
			db.insert("appointments_calendar", null, values);
			db.close();
	...
	public ArrayList<String> getCalendarEventIDs()
	{
		ArrayList<String> eventidArrayList=new ArrayList<String>();
		String query;
		if (syldatabase == null)
			syldatabase = new SYLDatabase(this.context);
		Cursor cursor = null;
		SQLiteDatabase db = syldatabase.getReadableDatabase();
		try {
			query = "select * from appointments_calendar";
			if (cursor == null)
				cursor = db
						.rawQuery(
								query,
			...
			String appointmentid=cursor.getString(cursor
					.getColumnIndex("appointment_id"));
			String calendarid=cursor.getString(cursor
					.getColumnIndex("event_id"));
		eventidArrayList.add(calendarid);
			Log.e("eventid",calendarid);
			}
		}
		catch(Exception e)
		{
